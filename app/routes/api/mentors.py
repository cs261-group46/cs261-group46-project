...
def index(user=None):
    ...
    if request.args:
        fields = [] if request.args.get('fields') is None else request.args.get('fields').split(',')
        filters = [] if request.args.get('filters') is None else request.args.get('filters').split(',')
    ...
    if "suitable" in filters:
        mentors = mentors.filter(Mentor.user.department_id != user.department_id).filter(Mentor.topics)
