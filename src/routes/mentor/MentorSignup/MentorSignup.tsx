...
const MentorSignup: FC<MentorSignupProps> = () => {
    ...
    const registrationHandler = () => {
        if (
            isSkillsValueValid && isProfileValueValid
        ) {
    ...
    return <MainLayout title={"Sign up to be a Mentor"}>

        <MultiSelect id={"interests"} label={"What are you interested in mentoring colleagues on?"} isValid={isSkillsInputValid} value={skills} onChange={skillsChangeHandler} onBlur={skillsBlurHandler}/>

        <TextInput id={"profile"} label={"Write a short profile about yourself, viewable by Mentors"} placeholder={"I can play Electric Guitar"} value={profile} isValid={isProfileInputValid} onChange={profileChangeHandler} onBlur={profileBlurHandler}/>
